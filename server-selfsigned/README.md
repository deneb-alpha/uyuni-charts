...
The proxy HTTP server needs a TLS certificate.
With the server using a certificate generated by cert-manager using a self-signed CA, the most convenient way to get the proxy certificate is to get cert-manager generate it too and copy it over to the proxy cluster.
...
Copy the generated secret to the proxy cluster in the same namespace the proxy will be installed in using a command like the following.
*Note that this command assumes `kubectl` is configured to access both the server and proxy clusters.*
